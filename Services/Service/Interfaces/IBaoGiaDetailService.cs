...
namespace PRJ_WAREHOUSE_BIVN.Services.Service.Interfaces
{
    public interface IBaoGiaDetailService: IBaseService<BaoGia_Detail_of_Quotation, int, BaoGia_Detail_of_QuotationDTO>
    {
        ...
        public Task<GenericResponse<ListRequest<dynamic>>> SearchBaoGiaAsync(int? idRequest, string? maDon, string? maVatTu, string? maNcc, string? section, DateTime? dayMM, int? PageSize, int? PageIndex);
        ...
        public Task<GenericResponse<int?>> GetIdOfQuotationAsync(string maDon, string maVatTu, string maNcc, string NameHQ);
        // update thông tin lựa chọn nhà  cung cấp
        public Task<GenericResponse<bool>> UpdatePickSupplierDetailAsync(List<BaoGia_Detail_of_QuotationDTO> dtos);
    }
}
