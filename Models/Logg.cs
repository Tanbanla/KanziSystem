...
namespace PRJ_WAREHOUSE_BIVN.Models
{
    public class KHO_NHAPXUAT
    {
        ...
        public static List<KHO_NHAPXUAT> _logg(string madon, string ngay_tu, string ngay_den, string kho, string manguyenlieu, string loai, string phong)
        {
            ...
            string timloai = $"Loai = '{loai}'";
            if(loai == "" || loai == null)
            {
                timloai = $"Loai like '%{loai}%'";
            }
            SQL_Connect_DB20 _db = new SQL_Connect_DB20();
            List<KHO_NHAPXUAT> lis_kho = new List<KHO_NHAPXUAT>();
            var lst = _db.GET_DATA_FROM_SQL($"SELECT TOP(500) * FROM [COST_MANAGEMENT].[dbo].[KHO_NHAPXUAT] where Hanhdong like '%{madon}%' {timngay} and Kho like '%{kho}%' and MaNguyenLieu like '%{manguyenlieu}%' and {timloai} and Phong like '%{phong}%' order by Id_Lichsu desc");
        ...
        public static List<KHO_NHAPXUAT> _truyxuat(string malinhkien)
        {
            SQL_Connect_DB20 _db = new SQL_Connect_DB20();
            List<KHO_NHAPXUAT> lis_kho = new List<KHO_NHAPXUAT>();
            var lst = _db.GET_DATA_FROM_SQL($"SELECT * FROM [COST_MANAGEMENT].[dbo].[KHO_NHAPXUAT] where MaNguyenLieu = '{malinhkien}' order by Id_Lichsu desc");
            for (int i = 0; i < lst.Rows.Count; i++)
            {
                lis_kho.Add(new KHO_NHAPXUAT
                {
