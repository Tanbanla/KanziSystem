     ...
     string? MaHang, string? status, int? step, string? user, int pageIndex, int pageSize, DateTime? to, DateTime? from, string? chungLoai)
        ...
        string? MaHang,string? user)
        {
            var sql = @"
                SELECT
                    COUNT(DISTINCT CASE
                        WHEN CAST(r.DTM_KyHan AS DATE) = CAST(GETDATE() AS DATE)
                        THEN CONCAT(r.CHR_MaDon, '|', r.CHR_MaHangNoiBo)
                    END) AS DenHanLuaChon,

                    COUNT(DISTINCT CASE
                        WHEN DATEDIFF(DAY, CAST(GETDATE() AS DATE), CAST(r.DTM_KyHan AS DATE)) = 1
                        THEN CONCAT(r.CHR_MaDon, '|', r.CHR_MaHangNoiBo)
                    END) AS ConMotNgayHetHan,
                    ...
                    COUNT(DISTINCT CASE
                        WHEN DATEDIFF(DAY, CAST(GETDATE() AS DATE), CAST(r.DTM_KyHan AS DATE)) < 0
                        THEN CONCAT(r.CHR_MaDon, '|', r.CHR_MaHangNoiBo)
                    END) AS QuaHan
                FROM BaoGia_Request_of_Quotation r
                WHERE r.BIT_LayBaoGia = 1
                  AND r.ID_StepBaoGia > 6
                  AND r.ID_StepBaoGia < 9
            ...
            string? MaHang, string? status, string? user, int pageIndex, int pageSize, DateTime? dateTo, DateTime? dateFrom, string? chungLoai)
        {
            ...
            switch (status)
            {
                case "RETURN":
                    ...
                    break;
                case "DONE":
                    finalStatusClause = "f.Step = 13 AND f.ID_Status NOT LIKE 'DELETE'";
                    break;
                case "APPROVAL":
                    finalStatusClause = "f.Step in (2,3,4,5,9,10,11) AND f.ID_Status NOT LIKE 'DELETE'";
                    break;
                case "WAIT":
                    finalStatusClause = "f.Step in (6,7,8) AND f.ID_Status NOT LIKE 'DELETE'";
                    break;
                case "DELETE":
            ...
            var sql = $@"
                ...
                ;WITH MAIN AS (
                    SELECT
                        f.CHR_MaDon,
                        f.GroupKey,
                        MAX(f.CHR_MaHangNoiBo) AS CHR_MaHangNoiBo,
                        f.NVCHR_ChungLoai,
                ...
                PICK AS (
                    ...
                    FROM (
                        SELECT
                            ...
                            d.FL_USD,
                            ROW_NUMBER() OVER (
                                PARTITION BY f.CHR_MaDon, f.GroupKey
                                ORDER BY
                                    CASE
                ...
                FINAL AS (
                    ...
                    LEFT JOIN NCC_PIVOT ncc
                        ...
                        AND m.GroupKey = ncc.GroupKey
                    LEFT JOIN HIS h
                        ON m.CHR_MaDon = h.CHR_MaDon
                        AND m.GroupKey = h.GroupKey
                    LEFT JOIN PICK p
